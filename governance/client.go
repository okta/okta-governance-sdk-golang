...
// OktaGovernanceAPIClient manages communication with the Okta Governance API API v3.2.0
// In most cases there should be only one, shared, OktaGovernanceAPIClient.
type OktaGovernanceAPIClient struct {
	...
	RequestSequencesAPI RequestSequencesAPI

	RequestSettingsAPI RequestSettingsAPI

	RequestTypesAPI RequestTypesAPI
...
type service struct {
	client *OktaGovernanceAPIClient
}
...
func NewAPIClient(cfg *okta.Configuration) *OktaGovernanceAPIClient {
...
func (c *OktaGovernanceAPIClient) callAPI(request *http.Request) (*http.Response, error) {
	if c.cfg.Debug {
...
func (c *OktaGovernanceAPIClient) GetConfig() *okta.Configuration {
	return c.cfg
}
...
func (c *OktaGovernanceAPIClient) prepareRequest(
	ctx context.Context,
	...
	if ctx != nil {
		...
		if tok, ok := ctx.Value(okta.ContextOAuth2).(oauth2.TokenSource); ok {
		...
		if auth, ok := ctx.Value(okta.ContextBasicAuth).(okta.BasicAuth); ok {
			localVarRequest.SetBasicAuth(auth.UserName, auth.Password)
		}

		// AccessToken Authentication
		if auth, ok := ctx.Value(okta.ContextAccessToken).(string); ok {
...
func (c *OktaGovernanceAPIClient) decode(v interface{}, b []byte, contentType string) (err error) {
	if len(b) == 0 {
		return nil
	}
	if s, ok := v.(*string); ok {
...
func (c *OktaGovernanceAPIClient) RefreshNext() *OktaGovernanceAPIClient {
	c.freshcache = true
	return c
}

func (c *OktaGovernanceAPIClient) do(ctx context.Context, req *http.Request) (*http.Response, error) {
...
func (c *OktaGovernanceAPIClient) parseLimitHeaders(resp *http.Response) (*RateLimit, error) {
